...
export default function Dashboard() {
  // Tipagem explícita para evitar o erro 'never' na Vercel
...
points.forEach((p, i) => {
      // Aceder à propriedade de forma segura com cast para 'any'
...
body: JSON.stringify({ value: novoValor }),
      ...
      console.log("Erro API local (esperado em produção)");
    }
...
{ label: "Accel Z", key: "z", val: lastData?.accel_z, color: "text-[#69ff47]", border: "border-l-[#69ff47]" },
          { label: "Temp (OpenWeather)", key: "t", val: lastData?.temperature, color: "text-[#ffab40]", border: "border-l-[#ffab40]", unit: "°C" }
...
<div className="text-[10px] text-[#4a5268] uppercase tracking-widest">{m.label}</div>
                {pred && <span className="text-[#00e5ff] text-[8px] font-bold tracking-tighter">PREVISÃO T+1</span>}
